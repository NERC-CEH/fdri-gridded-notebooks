...
## Getting started
Depending on the dataset you are interested in, the data format (NetCDF or Zarr) you want to use, and if you are a Python or R user will determine which notebook you first explore. If you are unsure if you should be using NetCDF or Zarr, then our [section about NetCDF and Zarr](#Learn-more-about-NetCDF-and-Zarr-data-formats-and-why-they-are-needed) should help you decide. If you are still unsure, then we suggest you use the [Zarr](https://zarr.dev/) versions as these can often be more efficient (in particular for Python users). If you are unsure if you should be using Python or R to explore and access gridded datasets, then we recommend you use Python as quicker to get started in Google Colab (installing R packages can take several minutes) and the packages e.g. [Xarray](https://xarray.dev/), at least for Zarr, are easier to use. In Google Colab, you may need to manually change the runtime for the R notebooks.
